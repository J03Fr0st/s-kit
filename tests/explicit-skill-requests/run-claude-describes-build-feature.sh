#!/usr/bin/env bash
# Test where Claude explicitly describes build-feature before user requests it
...
TIMESTAMP=$(date +%s)
OUTPUT_DIR="/tmp/s-kit-tests/${TIMESTAMP}/explicit-skill-requests/claude-describes"
mkdir -p "$OUTPUT_DIR"
...
# Turn 1: Have Claude describe execution options including build-feature
echo ">>> Turn 1: Ask Claude to describe execution options..."
claude -p "I have a plan at docs/specs/2026-05-27-auth-system/README.md. Tell me about my options for executing it, including what build-feature means and how it works." \
    --model haiku \
...
echo ">>> Turn 2: Request build-feature..."
FINAL_LOG="$OUTPUT_DIR/turn2.json"
claude -p "build-feature, please" \
    --continue \
    --model haiku \
    --plugin-dir "$PLUGIN_DIR" \
    --dangerously-skip-permissions \
...
# Check Turn 1 to see if Claude described build-feature
...
# Check final turn
SKILL_PATTERN='"skill":"([^"]*:)?build-feature"'
