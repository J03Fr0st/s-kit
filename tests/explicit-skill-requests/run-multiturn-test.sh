...
TIMESTAMP=$(date +%s)
OUTPUT_DIR="/tmp/s-kit-tests/${TIMESTAMP}/explicit-skill-requests/multiturn"
mkdir -p "$OUTPUT_DIR"
...
claude -p "Good analysis. I've already written the plan to docs/specs/2026-05-27-auth-system/README.md. Now I'm ready to implement. What are my options for execution?" \
    --continue \
    --plugin-dir "$PLUGIN_DIR" \
    --dangerously-skip-permissions \
    --max-turns 2 \
...
# Turn 3: The critical test - ask for build-feature
echo ">>> Turn 3: Requesting build-feature..."
...
if grep -q '"name":"Skill"' "$TURN3_LOG" && grep -qE "$SKILL_PATTERN" "$TURN3_LOG"; then
    echo "PASS: Skill 'build-feature' was triggered in Turn 3"
    TRIGGERED=true
else
    echo "FAIL: Skill 'build-feature' was NOT triggered in Turn 3"
    TRIGGERED=false
fi
