...
# 1. Create in s-kit location (lowest priority)
mkdir -p "$s-kit_SKILLS_DIR/priority-test"
cat > "$s-kit_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: s-kit version of priority test skill
---
# Priority Test Skill (s-kit Version)

This is the s-kit version of the priority test skill.

PRIORITY_MARKER_s-kit_VERSION
EOF
...
if [ -f "$s-kit_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] s-kit version exists"
else
    echo "  [FAIL] s-kit version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled s-kit priority
echo ""
echo "Test 2: Documenting personal vs s-kit priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled s-kit version instead of personal version"

# Test 3: Document project vs bundled s-kit priority
echo ""
echo "Test 3: Documenting project vs personal/s-kit priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled s-kit version instead of project version"

# Test 4: Test a non-colliding bundled s-kit skill is still available
echo ""
echo "Test 4: Testing non-colliding s-kit skill remains available..."

mkdir -p "$s-kit_SKILLS_DIR/s-kit-only-test"
cat > "$s-kit_SKILLS_DIR/s-kit-only-test/SKILL.md" <<'EOF'
---
name: s-kit-only-test
description: s-kit-only priority test skill
---
# s-kit Only Test Skill

PRIORITY_MARKER_s-kit_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"s-kit-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_s-kit_ONLY_VERSION" "Non-colliding s-kit skill is still registered"
