...
EOF
    ...
    cat > "$repo/assets/s-kit-small.svg" <<'EOF'
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/s-kit/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/s-kit/.fixture-keep"
    cat > "$repo/plugins/s-kit/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/s-kit/.fixture-keep
    git -C "$repo" add plugins/s-kit/skills/example/SKILL.md
...
add_openai_agent_metadata_fixture() {
    ...
    mkdir -p "$repo/plugins/s-kit/skills/example/agents"

    cat > "$repo/plugins/s-kit/skills/example/agents/openai.yaml" <<'EOF'
interface:
  display_name: "Example"
  short_description: "Destination-owned OpenAI metadata"
EOF

    git -C "$repo" add plugins/s-kit/skills/example/agents/openai.yaml
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/s-kit/.codex-plugin" \
        "$repo/plugins/s-kit/.private-journal" \
        "$repo/plugins/s-kit/assets" \
        "$repo/plugins/s-kit/skills/example/agents" \
        "$repo/plugins/s-kit/skills/example"

    cat > "$repo/plugins/s-kit/.codex-plugin/plugin.json" <<EOF
{
  "name": "s-kit",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/s-kit/assets/s-kit-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/s-kit/assets/app-icon.png"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/s-kit/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/s-kit/.codex-plugin/plugin.json \
        plugins/s-kit/assets/app-icon.png \
        plugins/s-kit/assets/s-kit-small.svg \
        plugins/s-kit/skills/example/agents/openai.yaml \
        plugins/s-kit/skills/example/SKILL.md \
        plugins/s-kit/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/s-kit/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/s-kit/.fixture-keep"
    printf 'stale ignored leak\n' > "$repo/plugins/s-kit/.private-journal/leak.txt"
    git -C "$repo" add plugins/s-kit/.fixture-keep
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/s-kit/skills/example/SKILL.md"
    noop_openai_metadata_path="$noop_apply_dest/plugins/s-kit/skills/example/agents/openai.yaml"
    ...
    assert_contains "$preview_section" "assets/s-kit-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (s-kit-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/s-kit-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/s-kit/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/s-kit" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/s-kit-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/s-kit'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/s-kit-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/s-kit-*" "Clean no-op local apply does not create sync branch in destination checkout"
