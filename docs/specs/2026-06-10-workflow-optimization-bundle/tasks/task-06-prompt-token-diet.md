...
1. **Coder template** (`coder-prompt-template.md`):
   - Remove the `## Feature Context` / `{requirements}`, `## Approved Design` / `{design}`, and `## Phase Risk Preflight` / `{Phase_risk_preflight}` sections from the template body.
   - Add in their place:
...
2. **Simplifier template** (`simplifier-prompt-template.md`):
   - Remove the `## Phase Risk Preflight` / `{Phase_risk_preflight}` section and its Placeholder Details entry.
   - Instruction 3 "Preserve contracts called out in the Phase Risk Preflight." becomes "Preserve contracts called out in the approved design and task summaries."
   ...
   - In the `{task_summaries}` placeholder details, change "task file content" to "task file content for this Phase's tasks" (current-Phase content is needed to judge intended behavior) and state that completed-task context from earlier Phases is one line per task.
...
3. **Review template** (`review-prompt-template.md`):
   ...
   - Keep `{requirements}` and `{design}` as full text (reviewers judge design conformance), keep `{Phase_risk_preflight}` (reviewers are the preflight's primary audience), keep `{read_only_contract}` from task-05, keep `{review_scope}` and the verdict format.
...
4. **`scripts/verify-workflow.ps1`** — update the template invariant blocks:
   - Coder template block (currently requires `'## Phase Risk Preflight'`, `'{Phase_risk_preflight}'`, `'Account for the Phase Risk Preflight contracts'`): require instead `'## Design Digest'`, `'{design_digest}'`, and `'Account for the contracts and conventions in the Design Digest'`; add negative checks that the coder template does NOT contain `'{Phase_risk_preflight}'`, `'{requirements}'`, or `'{design}'` (use `.Contains(...)` with a failure when found; note `'{design}'` must not false-positive on `'{design_digest}'` — check for `'{design}'` as an exact placeholder token, e.g. regex `\{design\}`).
   - Simplifier template block: drop the `'## Phase Risk Preflight'` / `'{Phase_risk_preflight}'` requirements; keep the no-op requirement and add `'you must still run each task''s Final Verification command'` (mind PowerShell single-quote escaping); add a negative check that `'{Phase_risk_preflight}'` is absent.
   - Review template block: keep `'{Phase_risk_preflight}'` and `'simplifier summary and verification evidence'` requirements; add `'Acceptance Criteria and Verification Plan sections'`.
...
- Spot check: `powershell -NoProfile -Command "-not (Select-String -Path 'skills/build-feature/references/coder-prompt-template.md','skills/build-feature/references/simplifier-prompt-template.md' -Pattern '\{Phase_risk_preflight\}' -Quiet)"` outputs `True`.
...
- [ ] Coder template: `{design_digest}` present with a placeholder spec (10–20 lines, contracts/naming/error-handling, quotes relevant preflight lines); `{requirements}`, `{design}`, `{Phase_risk_preflight}` absent.
